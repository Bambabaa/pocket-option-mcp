...
function buildMLFeatures(ind, state, opts = {}) {
    ...
    if (atr != null && atr > 0) {
        feat.body_atr        = body   / atr;              // unsigned: abs body size relative to ATR
        feat.signed_body_atr = (close - open) / atr;      // signed: negative = bearish candle
        feat.range_atr       = range  / atr;
        feat.upwick_atr      = upwick / atr;
        feat.dnwick_atr      = dnwick / atr;
    } else {
        feat.body_atr = feat.signed_body_atr = feat.range_atr = feat.upwick_atr = feat.dnwick_atr = null;
    }
...
const ML_ROUTER_KEYS = [
    'adx', 'atr_14', 'body_atr', 'signed_body_atr', 'dist_ema_26', 'dist_sma_20',
